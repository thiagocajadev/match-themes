...
function SwatchDot({ hue, chroma = 0.20, lightness = 0.65, label }: {
  hue: number;
  chroma?: number;
  lightness?: number;
  label?: string;
...
function HarmonyDots({ hues, labels }: { hues: number[]; labels?: string[] }) {
  return (
...
function ColorDots({ hues }: { hues: number[] }) {
  return (
  ...
  const layers = [
    {
      tag: 'S0',
      label: 'Background',
      detail: 'L 0.985 — plano base da página',
      bg: '#f5f5f4',
      indent: 0,
      shadow: false,
    },
    {
      ...
      detail: 'L 0.970 — painéis laterais',
      bg: '#e7e5e4',
      indent: 1,
      shadow: false,
    },
    ...
    },
    {
      tag: 'S3',
      label: 'Popover / Modal',
      detail: 'L 1.000 + sombra profunda',
      bg: '#ffffff',
      indent: 3,
      shadow: true,
    },
  ...
  return (
    <div className="mt-5 flex flex-col gap-2">
      {layers.map(({ tag, label, detail, bg, indent, shadow }) => (
        ...
        >
          <span className="font-mono text-xs font-bold text-stone-400 w-6 shrink-0">{tag}</span>
          <span className="font-semibold text-stone-800 text-sm sm:text-base">{label}</span>
          <span className="ml-auto text-xs text-stone-400 hidden sm:block">{detail}</span>
        </div>
      ))}
    </div>
  );
  ...
  return (
    <div className="mt-5 grid grid-cols-2 gap-3">
      ...
      <div
        ...
        style={{ background: 'oklch(0.165 0.006 106)' }}
      >
        ...
        <span className="mt-3 block font-mono text-[10px]" style={{ color: 'oklch(0.45 0.01 106)' }}>
          Dark — L 0.165
        </span>
      </div>
    </div>
...
function TonalScaleBar() {
  const SCALE = [
    { stop: 50,  l: 0.98, c: 0.01 },
    { stop: 100, l: 0.96, c: 0.03 },
    { stop: 200, l: 0.90, c: 0.06 },
    { stop: 300, l: 0.82, c: 0.09 },
    { stop: 400, l: 0.72, c: 0.12 },
    { stop: 500, l: 0.60, c: 0.15 },
    { stop: 600, l: 0.50, c: 0.14 },
    { stop: 700, l: 0.40, c: 0.12 },
    { stop: 800, l: 0.25, c: 0.09 },
    { stop: 900, l: 0.15, c: 0.05 },
    { stop: 950, l: 0.10, c: 0.03 },
  ] as const;
        ...
        {SCALE.map(({ stop, l, c }) => (
          ...
          >
            <span
              ...
              style={{ color: l > 0.5 ? 'oklch(0.22 0.05 250deg)' : 'oklch(0.92 0.01 250deg)' }}
            >
              {stop}
            </span>
          </div>
...
type TheoryEntry = { id: string; trigger: string; body: React.ReactNode };

const THEORY_ENTRIES: TheoryEntry[] = [
  {
    id: 'color-theory',
    ...
    body: (
      <div className="flex flex-col gap-6 text-base text-stone-700 sm:text-lg">
        ...
        <ul className="flex flex-col gap-4 border-l-2 border-stone-200 pl-5">
          <li>
            <strong>Primárias:</strong> vermelho, amarelo, azul. Não podem ser obtidas por
            mistura.
      ...
      <div className="flex flex-col gap-6 text-base text-stone-700 sm:text-lg">
        ...
        <div className="flex flex-col gap-5">
          ...
          <div className="rounded-xl border border-stone-200 bg-stone-50 p-5">
            ...
            <p className="mt-3">
              Quatro cores equidistantes (quadrado no círculo). Ainda mais variada que a
              tetrádica. Use apenas quando precisar de paletas muito ricas e sempre reduza o croma
              em 2 ou 3 das 4 cores para não sobrecarregar.
            </p>
  ...
  },
  {
    ...
    body: (
      <div className="flex flex-col gap-8 text-base text-stone-700 sm:text-lg">
        <p>
          Composição de cores é a arte de distribuir cores numa interface de forma que o olhar
          seja guiado, a hierarquia fique clara e o conjunto seja agradável. Três princípios
          fundamentais sustentam uma boa composição.
        </p>
        ...
        <div>
          ...
          <p className="mt-3">
            Distribua suas cores em proporções definidas: <strong>60%</strong> para a cor
            dominante (geralmente neutros ou cor primária suavizada), <strong>30%</strong> para a
            cor de suporte e <strong>10%</strong> para a cor de destaque. Essa proporção cria
            equilíbrio visual e direciona a atenção para os elementos certos.
          </p>
          <ColorBar603010 />
        </div>
        ...
        <div>
          ...
          <p className="mt-3">
            ...
            contraste em elementos de ação (botões primários, alertas) e cores neutras em
            elementos de suporte (bordas, fundos de seção, texto secundário).
          </p>
        </div>
        ...
        <div>
          ...
          <p className="mt-3">
            ...
            4.5:1. Em OKLCH, a diferença de L entre duas paradas é um preditor direto do
            contraste. Por isso, a escala tonal de 11 paradas deste estúdio gera combinações
            seguras.
          </p>
        </div>
    ...
    body: (
      <div className="flex flex-col gap-8 text-base text-stone-700 sm:text-lg">
        ...
        <p>
          ...
          dos produtos digitais busca AA. AAA é obrigatório apenas em contextos de alta
          criticidade, como saúde e serviços governamentais.
        </p>
        ...
        <div>
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-3">
            ...
            ] as const).map(({ level, normal, note, bg, border, color }) => (
              <div
                key={level}
        ...
        <div>
          ...
          <p className="mt-3">
            ...
            21:1 (branco sobre preto). Uma proporção de 4.5:1 significa que a cor mais clara é
            4.5 vezes mais luminosa que a mais escura.
          </p>
        </div>
        ...
        <div>
          ...
          <p className="mt-3">
            ...
            na maioria dos matizes. O badge de contraste em cada swatch deste estúdio faz o
            cálculo exato em sRGB para confirmar.
          </p>
          <p className="mt-4">
            O WCAG 3.0 (em desenvolvimento) deve adotar o método APCA (Advanced Perceptual
            Contrast Algorithm), que usa luminância perceptual próxima ao OKLCH. Paletas geradas
            em OKLCH já estão alinhadas com essa direção.
          </p>
        </div>
        ...
        <div>
          ...
          <p className="mt-3">
            ...
            conteúdo em hover (1.4.13) e movimento (2.3.3). Para paletas, os critérios mais
            diretos são 1.4.3 (contraste de texto), 1.4.6 (contraste AAA) e 1.4.11 (contraste de
    ...
    body: (
      <div className="flex flex-col gap-8 text-base text-stone-700 sm:text-lg">
        ...
        <div>
          ...
          <p className="mt-3">
            A arquitetura típica de um tema tem quatro níveis: <strong>background</strong> (o
            plano mais profundo), <strong>card / surface</strong> (painel elevado sobre o fundo),{' '}
        ...
        <div>
          ...
          <p className="mt-3">
            Interfaces com muitas cores saturadas em proximidade causam fadiga. A solução é
            reduzir o croma (saturação) das cores de fundo e reservar croma alto para elementos
            interativos e de destaque. Em OKLCH, manter croma abaixo de 0.05 em backgrounds
            garante neutralidade sem perder a tonalidade da paleta.
          </p>
        </div>
        ...
        <div>
          ...
          <p className="mt-3">
            Sombras não precisam ser pretas puras. Sombras tonalizadas com a cor base da paleta
            (um azul desaturado para um tema frio, por exemplo) integram melhor com o tema e
            parecem mais naturais. No dark mode, prefira diferença de luminosidade entre
            superfícies a sombras opacas, que desaparecem em fundo escuro.
          </p>
        </div>
      </div>
    ),
  },
  {
    ...
    body: (
      <div className="flex flex-col gap-8 text-base text-stone-700 sm:text-lg">
        <p>
          Criar um tema escuro de qualidade não é simplesmente inverter as cores do tema claro.
          São estratégias distintas que exigem atenção a como o olho humano percebe a luz em cada
          contexto.
        </p>
        ...
        <div>
          ...
          <p className="mt-3">
            ...
            0.12 e 0.18 para o background: escuro o suficiente para parecer dark, com ar
            suficiente para o conteúdo respirar.
          </p>
        </div>
        ...
        <div>
          ...
          <p className="mt-3">
            ...
            croma para saltar da superfície. Uma cor de botão primário que funciona com L = 0.55
            no light mode pode precisar de L = 0.65 a 0.70 no dark mode para manter o mesmo
            impacto visual.
          </p>
        </div>
        ...
        <div>
          ...
          <p className="mt-3">
            ...
            opacas escuras se fundem com o fundo. Use bordas claras com opacidade de 8 a 12%
            (como <code>oklch(1 0 0 / 10%)</code>) para separar cards sem criar peso visual
            excessivo.
          </p>
        </div>
    ...
    body: (
      <div className="flex flex-col gap-8 text-base text-stone-700 sm:text-lg">
        ...
        <div>
          ...
          <p className="mt-3">
            Em OKLCH, a parada 50 tem L aproximado de 0.97 e a parada 900 tem L aproximado de
            0.22. A diferença de 0.75 produz contraste WCAG muito acima de 7:1 (AAA)
            independentemente do matiz, graças à uniformidade perceptual do espaço OKLCH.
          </p>
        </div>
        ...
        <div>
          ...
          <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
            ...
            ] as const).map(({ bg, fg, bgStop, fgStop, label, pass }) => (
              <div
                key={bgStop + fgStop}
                className="flex flex-col overflow-hidden rounded-xl border"
                style={{
                  borderColor: pass ? 'oklch(0.75 0.15 145deg)' : 'oklch(0.75 0.18 25deg)',
                }}
              >
                <div
          ...
          <div className="mt-5 grid gap-3 sm:grid-cols-2">
            <div className="flex flex-col overflow-hidden rounded-xl border border-red-200">
              ...
              >
                <span
                  ...
                  style={{ color: 'oklch(0.97 0.01 95deg)' }}
                >
                  Texto claro
                </span>
                <span
            ...
            <div className="flex flex-col overflow-hidden rounded-xl border border-red-200">
              ...
              >
                <span
                  className="font-mono text-base font-bold"
                  style={{ color: 'oklch(0.97 0.01 200deg)' }}
                >
                  Texto claro
                </span>
                <span
            ...
            <ul className="flex flex-col gap-4">
              <li>
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="font-semibold text-stone-800 underline underline-offset-4 hover:text-stone-600"
                >
                  oklch.com
                </a>
          ...
          <div>
            ...
            <ul className="flex flex-col gap-4">
              ...
              <li>
                ...
                <span className="mt-1 block text-stone-500">
                  Série clássica em 3 partes cobrindo significado, temperatura e criação de
                  paletas
                </span>
              </li>
              <li>
              ...
              <li>
                ...
                <span className="mt-1 block text-stone-500">
                  Guia prático dos criadores do Tailwind CSS com capítulo dedicado a paletas e
                  contraste
                </span>
              </li>
            </ul>
  ...
  return (
    <section aria-labelledby={SECTION_ID} className="flex w-full flex-col gap-12">
      <div className="flex flex-col gap-4">
        ...
        >
          Guia de teoria e prática de cor
        </h2>
        <p className="max-w-3xl text-lg leading-relaxed text-stone-500 sm:text-xl">
          Tudo que você precisa saber para criar paletas com intenção, do círculo cromático às
          melhores combinações de paradas tonais para temas claro e escuro.
        </p>
        ...
        <div className="mt-4 flex flex-col items-center gap-10 sm:flex-row sm:items-start sm:gap-16">
          ...
          <div className="flex flex-col gap-4 pt-2 text-base leading-relaxed text-stone-500 sm:text-lg">
            <p>
              Todo matiz tem uma posição no círculo cromático. Harmonias surgem de relações
              geométricas entre essas posições: opostos criam tensão, vizinhos criam fluidez,
              equidistantes criam equilíbrio.
            </p>
            <p>
              No espaço OKLCH, o círculo é perceptualmente uniforme: cada salto de hue representa
              a mesma quantidade de mudança visual, independentemente do matiz. Isso torna as
              relações entre cores previsíveis e as harmonias matematicamente exatas.
            </p>
            <p>
              Os acordeões abaixo cobrem teoria, harmonias, composição, densidade visual, temas
              claro e escuro e dicas práticas de combinação de paradas tonais.
            </p>
          </div>
        </div>
      </div>
